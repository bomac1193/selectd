...
export function ProfileView({
  ...
  const [activeTab, setActiveTab] = useState<"stats" | "drops" | "selections">(
    "stats"
  );

  const winRate = profile
    ? profile.totalSelections > 0
      ? Math.round((profile.selectionWins / profile.totalSelections) * 100)
      : 0
    : 0;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-2 mb-6 border-b border-border">
        {(["stats", "drops", "selections"] as const).map((tab) => (
          <button
            key={tab}
          ...
          <div className="border border-border p-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-xl font-semibold">{profile?.selectionWins || 0}</p>
              </div>
              <div>
          ...
          {activeMissions.length > 0 && (
            <div>
              ...
              {activeMissions.slice(0, 3).map((mission) => (
                <div
                  key={mission.id}
                  className="border border-border p-3 mb-2"
                >
        ...
        <div className="space-y-3">
          {recentSelections.length > 0 ? (
            recentSelections.map((selection) => {
              const won = selection.winnerId === user.id;
              const isPlayer1 = selection.player1Id === user.id;

              return (
                <Link key={selection.id} href={`/selection/${selection.id}`}>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-16 flex items-center justify-center">
                        <span className="text-xs uppercase tracking-wider text-foreground/60">
                          {won ? "Win" : selection.winnerId ? "Loss" : "—"}
                        </span>
                      </div>
                      <div className="flex-1">
                        <p className="font-medium">
                          {isPlayer1 ? selection.track1.title : selection.track2?.title}{" "}
                          <span className="text-foreground/60">vs</span>{" "}
                          {isPlayer1 ? selection.track2?.title : selection.track1.title}
                        </p>
